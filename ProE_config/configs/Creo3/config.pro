...
mapkey gg @MAPKEY_LABEL初始;~ Command `ProCmdMdlTreeFilter` ;\
...
mapkey $F1 @MAPKEY_LABEL基准点显隐;~ Command `ProCmdEnvPntsDisp`  1;
mapkey $F2 @MAPKEY_LABEL基准轴显;~ Command `ProCmdEnvAxisDisp`  0;
mapkey $F9 @MAPKEY_LABEL待定...;;
mapkey $F10 @MAPKEY_LABEL制图-与视图相;~ Command `ProCmdDwgRelToView`;
mapkey $F11 @MAPKEY_LABEL制图-与对象相;~ Command `ProCmdDwgRelToObj`;
...
mapkey za @MAPKEY_LABEL装配中加入元;~ Command `ProCmdCompAssem` ;\
mapkey(continued) ~ Trail `UI Desktop` `UI Desktop` `DLG_PREVIEW_POST` `file_open`;\
mapkey(continued) ~ Trail `UI Desktop` `UI Desktop` `PREVIEW_POPUP_TIMER` \
...
mapkey $F3 @MAPKEY_LABEL基准面显;~ Command `ProCmdEnvDtmDisp`  0;
...
mapkey ~ @MAPKEY_LABEL工具栏收;~ Command `AdbCmdMinimizeRibbon`  1;
mapkey ce @MAPKEY_LABEL关闭
mapkey n @MAPKEY_LABEL退出模具模
...
mapkey rb @MAPKEY_LABEL轨迹
mapkey dg @MAPKEY_LABEL按尺寸移
